...
# 2) jq transform (same shape as coach)
cat > /tmp/xform.jq <<'"'"'JQ'"'"'
map({
...
| curl -fsS -X POST -H "Content-Type: application/json" -d @- \
    http://insight-agent.'"$NS"'.svc.cluster.local/api/fraud/detect \
| jq -C "{overall_risk, n_findings: (.findings|length), sample_finding: (.findings[0])}"
'
